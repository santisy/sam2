...
async def set_directory(request: dict):
    ...
    IMAGE_DIR = dir_path
    return {"success": True, "directory": str(IMAGE_DIR.absolute())}
...
def list_images():
    ...
    for ext in ["*.jpg", "*.jpeg", "*.png", "*.JPG", "*.JPEG", "*.PNG"]:
        for img_path in IMAGE_DIR.rglob(ext):
            ...
            images.append({"path": rel_path, "filename": img_path.name, "mask_count": mask_count})
...
async def predict_mask(request: dict):
    ...
    return {"mask": mask.tolist(), "shape": mask.shape}
...
async def create_mask(request: dict):
    ...
    return mcs.create_mask(IMAGE_DIR, img_path, mask_array, request['prompt'])
...
async def update_mask(request: dict):
    ...
    mask_array = np.array(request['mask'], dtype=bool)
    return mcs.update_mask(IMAGE_DIR, img_path, request['mask_index'], mask_array, request['prompt'])
...
async def delete_mask(request: dict):
    ...
    return mcs.delete_mask(IMAGE_DIR, img_path, request['mask_index'])
...
async def save_crop(request: dict):
    ...
    return mcs.save_crop(IMAGE_DIR, img_path, request['crop'])
...
async def delete_crop(request: dict):
    ...
    return mcs.delete_crop(IMAGE_DIR, img_path)
...
async def _generate(request: dict, model: str):
    return vgs.submit_job(IMAGE_DIR, request, model)


@app.post("/api/generate/sora2")
async def generate_sora2(request: dict):
    return await _generate(request, 'sora2')


@app.post("/api/generate/veo3")
async def generate_veo3(request: dict):
    return await _generate(request, 'veo3')


@app.post("/api/generate/kling")
async def generate_kling(request: dict):
    return await _generate(request, 'kling')
...
@app.get("/api/config")
def get_config():
    return {"image_dir": str(IMAGE_DIR.absolute()), "total_images": len(list_images())}


def _serve_video(model_dir_name: str, filename: str):
    video_dir = IMAGE_DIR / "annotations" / model_dir_name
...
def serve_our_video(filename):
    return _serve_video("ours", filename)


@app.get("/api/video/sora2/{filename}")
def serve_sora2_video(filename):
    return _serve_video("sora2", filename)


@app.get("/api/video/veo3/{filename}")
def serve_veo3_video(filename):
    return _serve_video("veo3", filename)


@app.get("/api/video/kling/{filename}")
def serve_kling_video(filename):
